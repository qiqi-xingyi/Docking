...
class Fileprepare:
    ...
    def __init__(self, xyz_file, docking_folder=None):
        ...
        # Determine xyz file directory
        self.base_dir = os.path.dirname(os.path.abspath(self.xyz_file))
        print("XYZ base directory:", self.base_dir)

        # Generate intermediate filenames based on xyz file
        self.ca_pdb_filename = os.path.join(self.base_dir, 'ca_model.pdb')  # Cα-only PDB
        self.alignment_filename = os.path.join(self.base_dir, 'protein.ali')  # Alignment file
        self.full_model_filename = os.path.join(self.base_dir, 'full_model.pdb')  # Full-atom PDB generated by MODELLER

        # Final PDBQT filename (initially in base_dir, can be moved to docking_folder)
        self.pdbqt_filename = self.full_model_filename.replace(".pdb", ".pdbqt")

        # Save final PDBQT file to docking_folder if specified; otherwise, default to base_dir
        ...
        # Set chain_id using the fifth-last character of the xyz file's name (modify if needed)
